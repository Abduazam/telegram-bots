...
use App\Services\Bots\Taskable\Tasks\Task\TaskableTaskCreateService;
use App\Services\Bots\Taskable\Tasks\Task\TaskableTaskDeleteService;
use App\Services\Bots\Taskable\Tasks\Task\TaskableTaskRestoreService;
use App\Services\Bots\Taskable\Tasks\Task\TaskableTaskUpdateService;
use App\Services\Bots\Taskable\Tasks\Track\TaskableTaskTrackCreateService;
...
class TaskableController extends BotsController
{
    ...
    public function index(): void
    {
        if ($this->user->isActive()) {
             ...
             * Done button.
             */
            if (preg_match('/^task-done-button@(\d+)/', $this->text, $matches) and $this->message_type === $this->telegram::CALLBACK_QUERY) {
                $task_id = $matches[1];

                $this->telegram->deleteMessage(['chat_id' => $this->user->chat_id, 'message_id' => $this->message_id]);
                $this->user->updateSteps(6, 0);
                (new TaskableLogCreateService($this->user, $task_id))->createByTaskId();
                $this->telegram->sendMessage(Message::taskCompleteMessage($this->user, $task_id));
            } else {
                /**
                ...
                /**
                 * Entering completed task count
                 */
                if ($this->step_one === 6) {
                    if ($this->step_two === 0) {
                        if (is_numeric($this->text)) {
                            $result = (new TaskableTaskTrackCreateService($this->user, $this->text));
                            if ($result()) {
                                $this->user->updateSteps(1, 0);
                                $this->telegram->sendMessage(Message::taskCountSaved($this->user, $this->text));
                            } else {
                                $this->telegram->sendMessage(Message::somethingWentWrong($this->user->chat_id));
                            }
                        }
